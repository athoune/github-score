...
# Dependabot security PRs carry this marker in their body (generated by
# dependabot-core); plain version bumps only mention "security" inside the
# dependency changelog, so the marker is required to tell them apart.
_SECURITY_UPDATE_MARKERS = (
    "this update includes a security fix",
    "this update includes security fixes",
)


def _is_security_update(body: str) -> bool:
    """True when a Dependabot PR body fixes security advisories."""
    lowered = body.lower()
    return any(marker in lowered for marker in _SECURITY_UPDATE_MARKERS)
...
class GitHubFetcher:
    ...
    async def fetch_security_updates(self, url: RepoUrl) -> list[SecurityUpdate]:
        """Open Dependabot-family pull requests fixing security advisories.

        Detection relies on the body marker alone (generated by
        dependabot-core and reused by derivative bots such as
        ``dotnet-updater[bot]``), not on the author login: plain version
        bumps mention "security" only inside the dependency changelog, so
        the marker is the discriminating signal.
        """
        data = await self._get_all_pages(
            f"{url.api_url}/pulls",
            params={"state": "open"},
        ...
        for pr in data:
            if not isinstance(pr, dict):
                continue
            if not _is_security_update(pr.get("body") or ""):
                continue
